...
namespace Docnet.Core
{
    public sealed class DocLib : IDocLib
    {
        ...
        private DocLib()
        {
            //CHANGED,之前以为需要先调用其它的以及依赖的so库,但最后发现不需要
            //Java.Lang.JavaSystem.LoadLibrary("c++_chrome.cr");
            //Java.Lang.JavaSystem.LoadLibrary("icuuc.cr");
        ...
        /// <returns></returns>
        public IDocReader GetDocReader(Stream stream, string password)
        {
            //TODO:验证流是否为空
            return new DocReader(stream, password);
        }
