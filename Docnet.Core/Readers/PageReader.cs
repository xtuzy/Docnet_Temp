    ...
    internal sealed class PageReader : IPageReader
    {
        ...
        public int GetPageWidth()
        {
            lock (DocLib.Lock)
            {
                ...
                return (int)(fpdf_view.FPDF_GetPageWidth(_page));
            }
        }
        ...
        public int GetPageHeight()
        {
            lock (DocLib.Lock)
            {
                ...
                return (int)(fpdf_view.FPDF_GetPageHeight(_page));
            }
        }
        ...
        /// <inheritdoc />
        public byte[] GetImage() => GetImage(RenderFlags.RenderAnnotations);

        /// <inheritdoc />
        public byte[] GetImage(int dpi)
        {
            lock (DocLib.Lock)
            {
                // Get Metrics
                ...
                int width = (int)(GetPageWidth() * dpi / 72);
                int height = (int)(GetPageHeight() * dpi / 72);
                Debug.WriteLine("GetImage", $"DPI:{dpi},Size:{width },{height}");
                ...
                try
                {
                    ...
                    using (var clipping = new FS_RECTF_())
                    {
                        ...
                        matrix.A = (float)1*dpi / 72;
                        matrix.B = 0;
                        matrix.C = 0;
                        //matrix.D = (float)_scaling;
                        matrix.D = (float)1 * dpi / 72;
                        matrix.E = 0;
                        matrix.F = 0;
                        ...
                        fpdf_view.FPDF_RenderPageBitmapWithMatrix(bitmap, _page, matrix, clipping, (int)RenderFlags.RenderAnnotations);
        ...
        public int GetPageWidthPixel()
        {
            ...
            var mainDisplayInfo = DeviceDisplay.MainDisplayInfo;
            var density = mainDisplayInfo.Density;
            int width = (int)(GetPageWidth() * density * 160 / 72);
            return width;
        }
        /// <inheritdoc />
        public int GetPageHeightPixel()
        {
            ...
            var mainDisplayInfo = DeviceDisplay.MainDisplayInfo;
            var density = mainDisplayInfo.Density;
            int height = (int)(GetPageHeight() * density * 160 / 72);
            return height;
        }
    }
}
